...
--- Loads and provides Lua module. Mimics standard
--- 'require' behavior, but loads only Lua scripts and uses current SASL project
...
function request(name)
    ...
    local f = openFile(name)
    if not f then
        logError("Can't request script "..name)
        return nil
    else
        _SLOADED[name] = _SGUARD
        local r = f()
        if r then
            _SLOADED[name] = r
        end
